...
class GroupListAdapter(
    ...
    inner class GroupViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: GroupItem) {
            name.text = item.name
            when (item.role) {
                "owner" -> { role.text = "所有者"; role.setBackgroundResource(R.drawable.bg_role_owner) }
                "editor" -> { role.text = "编辑者"; role.setBackgroundResource(R.drawable.bg_role_editor) }
                else -> { role.text = "查看者"; role.setBackgroundResource(R.drawable.bg_role_viewer) }
            }
            meta.text = "${item.memberCount} 成员 · ${item.storageUsed}"
            visibilityIcon.setImageResource(if (item.visibility == "private") android.R.drawable.ic_lock_lock else android.R.drawable.presence_online)
